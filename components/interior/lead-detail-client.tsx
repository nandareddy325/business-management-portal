    ...
    <div style={{ background: '#F5F0E8', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1C1712, #2d2218)' }}>
        <div className="p-4 md:p-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <a href={`tel:${lead.phone}`} className="font-mono text-sm mt-0.5 block" style={{ color: '#F59E0B' }}>{lead.phone}</a>
        ...
        <div className="px-4 md:px-6 pb-5">
          <p className="text-[9px] font-bold uppercase tracking-[2px] mb-3" style={{ color: 'rgba(255,255,255,0.4)' }}>Pipeline Progress</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={x.label} className="bg-white rounded-2xl p-4 border border-[#E8E2D8] shadow-sm">
              ...
              <p className="text-sm font-bold text-[#1C1712]">{x.icon} {x.value}</p>
            </div>
          ))}
        </div>
          ...
          <div className="p-4 flex flex-wrap gap-2">
            {PIPELINE_STAGES.map(stage => (
              <button key={stage.key} onClick={() => handleStageChange(stage.key)} disabled={savingStage}
                ...
                {lead.pipeline_stage === stage.key && <span className="text-[8px] opacity-70">✓</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E8E2D8] shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-[#F0EBE0] flex items-center justify-between" style={{ background: '#FAFAF8' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowNoteModal(true)}
                className="text-[10px] font-bold px-2.5 py-1 rounded-lg text-white" style={{ background: '#1C1712' }}>
                + Add
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-[#E8E2D8] overflow-hidden">
            <div className="px-5 py-4 border-b border-[#F0EBE0] flex items-center justify-between" style={{ background: '#FAFAF8' }}>
              ...
              <button onClick={() => setShowNoteModal(false)} className="w-7 h-7 rounded-full bg-[#F5F0E8] flex items-center justify-center text-[#9A8F82]">✕</button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowNoteModal(false)} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-[#7A6E60] border border-[#E8E2D8]">Cancel</button>
