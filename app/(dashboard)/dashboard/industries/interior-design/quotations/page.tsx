...
export default async function QuotationsPage() {
  ...
  const avgBudget    = count && count > 0 ? Math.round(totalBudget / count) : 0
  ...
  return (
    <div className="space-y-5 p-4 md:p-6" style={{ background: '#F5F0E8', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#9A8F82] mt-0.5">
            <span className="font-bold text-[#1C1712]">{count ?? 0}</span> leads — quotation pampinchamu
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Quotes',   value: String(count ?? 0),   color: '#DB2777', isText: false },
      ...
      {avgBudget > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
          ...
          <FileText className="w-4 h-4 flex-shrink-0" style={{ color: '#B8860B' }} />
          <p className="text-sm font-medium" style={{ color: '#92400E' }}>
            Average quotation value:{' '}
            <span className="font-black" style={{ color: '#B8860B' }}>{fmt(avgBudget)}</span>
            {' '}— close cheyyandi!
          </p>
        </div>
      )}
      ...
      {!leads?.length ? (
        <div className="bg-white border border-[#E8E2D8] rounded-2xl py-20 text-center shadow-sm">
          ...
          <p className="text-[#1C1712] font-bold text-base">No quotations yet</p>
          <p className="text-[#9A8F82] text-sm mt-1">Leads move here after quotation is sent</p>
        </div>
      ) : (
        <div className="bg-white border border-[#E8E2D8] rounded-2xl overflow-hidden shadow-sm">
                ...
                {leads.map((l: any, i: number) => {
                  ...
                  return (
                    <tr key={l.id} className="border-b border-[#F7F5F1] last:border-0 hover:bg-[#FDFAF8] transition-colors cursor-pointer">
                      ...
                      <td className="px-4 py-3.5">
                        {l.budget ? (
                          <p className="text-2xl font-black" style={{ color: '#B8860B', fontSize: '0.95rem' }}>
                            {fmt(Number(l.budget))}
                          </p>
                        ) : <span className="text-[#C4BAB0]">—</span>}
                      </td>
              ...
              {totalBudget > 0 && (
                <tfoot>
                  <tr style={{ background: '#F5F0E8', borderTop: '2px solid #E8E2D8' }}>
                    <td colSpan={5} className="pl-5 py-3 text-xs font-black text-[#7A6E60] uppercase tracking-wider">
                    ...
                    <td colSpan={3} className="pr-5 py-3 text-right">
                      <p className="text-[10px] text-[#B8B0A0]">{count} quotations</p>
                    </td>
                  </tr>
                </tfoot>
              )}
          ...
          <div className="md:hidden divide-y divide-[#F0EBE0]">
            ...
            {totalBudget > 0 && (
              <div className="px-4 py-3 flex items-center justify-between" style={{ background: '#F5F0E8' }}>
                <p className="text-xs font-black text-[#7A6E60] uppercase tracking-wider">Total Pipeline</p>
                <p className="text-base font-black" style={{ color: '#B8860B' }}>
                  {totalBudget >= 100000
                    ? '₹' + (totalBudget / 100000).toFixed(2) + 'L'
                    : fmt(totalBudget)}
          ...
          <div className="px-5 py-3 border-t border-[#F0EBE0] flex items-center justify-between"
            ...
            <p className="text-[10px] text-[#9A8F82]">
              <span className="font-bold text-[#1C1712]">{count ?? 0}</span> quotations sent
            </p>
