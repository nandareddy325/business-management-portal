    ...
    <div className="space-y-5 p-4 md:p-6" style={{ background: '#F5F0E8', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#1C1712]">Calling</h1>
          <p className="text-sm text-[#9A8F82] mt-0.5">
            <span className="font-bold text-[#1C1712]">{count ?? 0}</span> leads — anni calling jarigindi
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Calling',  value: String(count ?? 0),   color: '#2563EB', isText: false },
      ...
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
        ...
        <p className="text-sm text-blue-700 font-medium">
          Ee leads ki call chesam — response kosam wait chestunnamu
        </p>
      </div>
      ...
      {!leads?.length ? (
        <div className="bg-white border border-[#E8E2D8] rounded-2xl py-20 text-center shadow-sm">
          <div className="w-16 h-16 bg-[#F5F0E8] border border-[#E2D9C8] rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Phone className="w-8 h-8 text-[#B8860B]" />
          </div>
          <p className="text-[#1C1712] font-bold text-base">No leads in calling stage</p>
          <p className="text-[#9A8F82] text-sm mt-1">Move leads here after initiating a call</p>
        </div>
      ) : (
        <div className="bg-white border border-[#E8E2D8] rounded-2xl overflow-hidden shadow-sm">
                  ...
                  return (
                    <tr key={l.id} className="border-b border-[#F7F5F1] last:border-0 hover:bg-[#FDFAF8] transition-colors cursor-pointer">
                      ...
                      <td className="px-4 py-3.5">
                        <a href={`tel:${l.phone}`}
                          className="flex items-center gap-1.5 text-sm font-mono font-bold text-blue-600 hover:text-blue-800 transition-colors">
                ...
                <div key={l.id} className="px-4 py-4 hover:bg-[#FDFAF8] transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Clickable phone */}
                      <a href={`tel:${l.phone}`}
                        className="flex items-center gap-1 text-xs font-bold text-blue-600 mt-0.5">
                        <Phone className="w-3 h-3" />
                        {l.phone ?? '—'}
                      </a>
          ...
          <div className="px-5 py-3 border-t border-[#F0EBE0] flex items-center justify-between"
            ...
            <p className="text-[10px] text-[#9A8F82]">
              <span className="font-bold text-[#1C1712]">{count ?? 0}</span> leads in calling
            </p>
