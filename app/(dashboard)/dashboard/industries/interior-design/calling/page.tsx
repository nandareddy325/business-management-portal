  ...
  return (
    <div className="space-y-5 p-4 md:p-6" style={{ background: '#F5F0E8', minHeight: '100vh' }}>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold" style={{ background: '#EFF6FF', color: '#2563EB', border: '1px solid #BFDBFE' }}>
          <Phone className="w-4 h-4" />Calling Stage
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total Calling',  value: String(count ?? 0), color: '#2563EB' },
      ...
      <LeadTable leads={leads ?? []} count={count ?? 0} footerText="calling leads" emptyIcon="📞" emptyText="No leads in calling stage" showCall={true} columns={['#', 'Lead', 'Phone', 'Source', 'Interest', 'Budget', 'City', 'Date']} />
    </div>
  )
}