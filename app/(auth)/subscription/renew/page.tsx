    ...
    async function loadCompany() {
      ...
      if (profile?.company_id) {
        ...
        if (sub?.trial_ends_at) {
          const diff = Math.floor((Date.now() - new Date(sub.trial_ends_at).getTime()) / (1000 * 60 * 60 * 24))
  ...
  async function handlePayment() {
    ...
    try {
      const res = await fetch('/api/razorpay/create-order', {
        ...
        body: JSON.stringify({ amount: PLAN_PRICE * 100, companyId, planConfig: { 'interior-design': 'pro' } }),
      })
      ...
      const options = {
        ...
        name: 'GK CRM', description: 'Interior Design — Professional Plan',
        order_id: order.id,
        ...
        handler: async (response: any) => {
          try {
            await fetch('/api/razorpay/verify', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ ...response, companyId, planConfig: { 'interior-design': 'pro' } }),
            })
            ...
            setError('Activation failed. Contact support.'); setLoading(false)
          }
        },
        modal: { ondismiss: () => setLoading(false) },
      }
      const rzp = new window.Razorpay(options)
      rzp.on('payment.failed', (r: any) => { setError('Payment failed: ' + r.error.description); setLoading(false) })
      setLoading(false); rzp.open()
    } catch (err: any) {
          ...
          <div style={{ width: '100%', maxWidth: 480 }}>
            ...
            <div className="slide-up" style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#FEF3C7', border: '1px solid #FCD34D', borderRadius: 20, padding: '4px 14px', marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: '#92400E', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Trial Ended</span>
              </div>
              <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 36, fontWeight: 700, color: '#1C1712', margin: '0 0 10px', lineHeight: 1.2 }}>
                Your free trial<br />
                <span style={{ color: '#B8860B', fontStyle: 'italic' }}>has ended</span>
              </h1>
              {companyName && (
                <p style={{ fontSize: 14, color: '#7A6E60', margin: 0 }}>
                  <strong style={{ color: '#1C1712' }}>{companyName}</strong>
                  {daysExpired > 0 && <span style={{ color: '#9A8F82' }}> · {daysExpired} day{daysExpired > 1 ? 's' : ''} ago</span>}
                </p>
              )}
            </div>
            ...
            <div className="slide-up" style={{ background: '#1C1712', borderRadius: 20, padding: 24, boxShadow: '0 8px 40px rgba(28,23,18,0.15)' }}>
              ...
              <button onClick={handlePayment} disabled={loading || !companyId}
                ...
                {loading ? '⏳ Processing...' : `Pay ${fmt(PLAN_PRICE)} & Continue →`}
              </button>
