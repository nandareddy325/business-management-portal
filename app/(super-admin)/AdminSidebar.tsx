  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    ...
    `}>
      ...
      `}>
        ...
        {(!collapsed || mobile) && (
          <div>
            <p className="text-sm font-bold text-white tracking-tight leading-none">GK · CRM</p>
            <p className="text-[10px] text-amber-400/80 font-semibold tracking-widest uppercase mt-0.5">Super Admin</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        {!collapsed || mobile ? (
          <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest px-3 mb-3">Navigation</p>
        ...
        {NAV.map(({ href, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              ...
              <Icon size={17} className={`flex-shrink-0 ${active ? 'text-amber-400' : ''}`} />
              {(!collapsed || mobile) && (
              ...
              {collapsed && !mobile && (
                <div className="absolute left-full ml-3 px-2.5 py-1.5 bg-gray-800 border border-white/10 rounded-lg text-xs font-medium text-white whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-xl">
                  {label}
                </div>
              )}
            </Link>
      ...
      <div className="px-2 pb-4 space-y-0.5 border-t border-white/5 pt-3">
        ...
        <form action="/api/auth/signout" method="POST">
          <button
            ...
            className={`
              w-full flex items-center gap-3 rounded-xl px-3 py-2.5
              text-white/30 hover:text-red-400 hover:bg-red-500/5 transition-all group
              ${collapsed && !mobile ? 'justify-center px-0' : ''}
            `}
          >
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 w-9 h-9 rounded-xl bg-gray-900 border border-white/10 flex items-center justify-center text-white/60 hover:text-white shadow-lg"
      >
        <Menu size={18} />
      </button>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          />
          {/* Drawer */}
          <div className="relative flex flex-col bg-[#0D0D10] border-r border-white/5 shadow-2xl animate-slide-in-left">
            {/* Close btn */}
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-4 right-4 w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center text-white/40 hover:text-white"
            >
              <X size={14} />
            </button>
