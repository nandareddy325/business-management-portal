...
export default async function SuperAdminDashboard() {
  ...
  // No role check here — layout.tsx already handles it
  const stats = await getSystemStats(supabase)

  const statCards = [
    { label: 'Total Companies', value: stats.totalCompanies, icon: Building2, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
    { label: 'Total Users',     value: stats.totalUsers,     icon: Users,     color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' },
    { label: 'Total Leads',     value: stats.totalLeads,     icon: TrendingUp, color: 'text-[#B8860B]', bg: 'bg-amber-50', border: 'border-amber-100' },
    { label: 'Active Companies',value: stats.activeCompanies,icon: Activity,  color: 'text-violet-600', bg: 'bg-violet-50', border: 'border-violet-100' },
  ]
  ...
  return (
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <h1 className="font-serif text-2xl text-white">System Dashboard</h1>
        <p className="text-sm text-gray-400 mt-1">Platform-wide overview — all tenants & health</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => {
          ...
          return (
            <div key={card.label} className={`rounded-2xl border ${card.border} ${card.bg} p-5 flex flex-col gap-3`}>
      ...
      <div className="rounded-2xl border border-white/10 bg-gray-800 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-serif text-base text-white">System Health</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-gray-700">
      ...
      <div className="rounded-2xl border border-white/10 bg-gray-800 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-base text-white">Recent Companies</h2>
          </div>
          <a href="/super-admin/admin/tenants" className="text-xs font-semibold text-[#B8860B] hover:underline">View all →</a>
        </div>
        ...
        {stats.recentCompanies.length === 0 ? (
          ...
          <div className="space-y-2">
            {stats.recentCompanies.map((company: { id: string; name: string; created_at: string; is_active: boolean }) => (
              <div key={company.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-700 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-gray-700 border border-white/10 flex items-center justify-center">
                ...
                <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${company.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <a key={link.label} href={link.href}
            className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-white/10 bg-gray-800 hover:bg-gray-700 hover:border-amber-500/40 transition-all text-center">
