        ...
        <div className="bg-white rounded-2xl border border-[#E8E2D8] shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[#F0EBE0]">
                ...
                <tr key={tenant.id} className="hover:bg-[#FFFBEF] transition-colors">
                  ...
                  <td className="px-5 py-3">
                    ...
                    {tenant.plan?.price_monthly && (
                      <p className="text-xs text-[#9A8F82]">₹{tenant.plan.price_monthly}/mo</p>
                    )}
                  </td>
                  <td className="px-5 py-3">
              ...
              {!tenants.length && (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-[#9A8F82] text-sm">No tenants found</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <a href={`?page=${page - 1}&search=${searchParams.search ?? ''}`}
                  className="px-3 py-1.5 text-xs bg-white border border-[#E8E2D8] text-[#1C1712] rounded-lg hover:border-[#B8860B] transition-colors">← Prev</a>
              )}
              {page < totalPages && (
                <a href={`?page=${page + 1}&search=${searchParams.search ?? ''}`}
                  className="px-3 py-1.5 text-xs bg-white border border-[#E8E2D8] text-[#1C1712] rounded-lg hover:border-[#B8860B] transition-colors">Next →</a>
              )}
            </div>
          </div>
        )}
